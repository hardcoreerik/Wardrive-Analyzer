...
esp_err_t bsp_touch_new(const bsp_touch_config_t *config, esp_lcd_touch_handle_t *ret_touch)
{
    ...
    /*
     * Tab5 shipped with two touch controller generations: older units use
     * a discrete GT911, newer units use the integrated ST7123 TDDI chip
     * (confirmed via I2C probe: GT911 doesn't respond at all on those
     * units, ST7123 responds at 0x55). Try GT911 first to keep older
     * units working, fall back to ST7123 if that fails.
     */
    esp_lcd_panel_io_handle_t tp_io_handle = NULL;
    esp_lcd_panel_io_i2c_config_t gt911_io_config = ESP_LCD_TOUCH_IO_I2C_GT911_CONFIG();
    gt911_io_config.dev_addr = ESP_LCD_TOUCH_IO_I2C_GT911_ADDRESS_BACKUP;
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_i2c(i2c_handle, &gt911_io_config, &tp_io_handle), TAG, "");

    esp_err_t gt911_err = esp_lcd_touch_new_i2c_gt911(tp_io_handle, &tp_cfg, ret_touch);
    if (gt911_err == ESP_OK) {
        return ESP_OK;
    }
    ESP_LOGW(TAG, "GT911 not found, trying ST7123");
    esp_lcd_panel_io_del(tp_io_handle);

    tp_io_handle = NULL;
    esp_lcd_panel_io_i2c_config_t st7123_io_config = ESP_LCD_TOUCH_IO_I2C_ST7123_CONFIG();
    ESP_RETURN_ON_ERROR(esp_lcd_new_panel_io_i2c(i2c_handle, &st7123_io_config, &tp_io_handle), TAG, "");

    ESP_RETURN_ON_ERROR(esp_lcd_touch_new_i2c_st7123(tp_io_handle, &tp_cfg, ret_touch), TAG,
                        "New touch driver initialization failed (tried GT911 and ST7123)");
...
lv_display_t *bsp_display_start_with_config(const bsp_display_cfg_t *cfg)
{
    ...
    // Touch is not fatal to display bring-up: some Tab5 revisions ship the
    // newer ST7123 integrated display-touch controller instead of GT911,
    // which this driver doesn't support yet (probe it via touch::probe()
    // in the app to detect which chip is actually present). A missing/
    // failed touch controller shouldn't take down an otherwise-working
    // screen -- fail soft and let the caller check bsp_display_get_input_dev().
    disp_indev_touch = bsp_display_indev_touch_init(disp);
    if (disp_indev_touch == NULL) {
        ESP_LOGW(TAG, "touch controller init failed, continuing with display-only (no touch input)");
    }
